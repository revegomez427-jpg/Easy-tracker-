...
const THEMES = {
  ...
  cafe: {
    name: 'Café', icon: '☕',
    bg: '#12110f', card: '#1c1a17', elevated: '#241f1a', border: '#2e2a24',
    lime: '#c9a15a', slate: '#9a9285', white: '#f3efe7',
    danger: '#d9704f', warn: '#c9a15a', isLight: false,
    good: '#6fa582',
  },
}
const THEME_ORDER = ['dark', 'light', 'cafe']
...
// ============================================================
// ============================================================
// ÍCONO DE TENDENCIA (línea + flecha, estilo mini gráfico)
// ============================================================
function TrendIcon({ direction, color, size = 20 }) {
  if (direction === 'stable') {
    return (
  ...
  const points = direction === 'up' ? '3,17 9,11 13,15 21,5' : '3,7 9,13 13,9 21,19'
  const arrowPoints =
    direction === 'up'
      ? { tip: [21, 5], p1: [14, 5], p2: [21, 12] }
      : { tip: [21, 19], p1: [21, 12], p2: [14, 19] }
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
      <polyline points={points} stroke={color} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
      <polyline
        points={`${arrowPoints.p1[0]},${arrowPoints.p1[1]} ${arrowPoints.tip[0]},${arrowPoints.tip[1]} ${arrowPoints.p2[0]},${arrowPoints.p2[1]}`}
...
// VISTA: AGREGAR GASTO
// ============================================================
function AddView({ form, setForm, addExpense, addTransfer, goals, etMood, remColor, remaining, i, T }) {
  const [mode, setMode] = useState('expense') // 'expense' | 'transfer'
  const [transferGoalId, setTransferGoalId] = useState(goals[0]?.id || '')
  const [transferAmount, setTransferAmount] = useState('')
  const [transferNote, setTransferNote] = useState('')
  const inputStyle = { width: '100%', padding: '0.8rem 1rem', background: T.border, border: `1.5px solid ${T.border}`, borderRadius: 12, color: T.white, fontSize: '0.95rem', outline: 'none', boxSizing: 'border-box', fontFamily: 'inherit' }

  function handleTransferSubmit() {
    const ok = addTransfer(transferGoalId, transferAmount, transferNote)
    if (ok) {
      setTransferAmount('')
      setTransferNote('')
    }
    ...
    <div style={{ padding: '1.5rem 1.25rem', paddingBottom: '6rem', background: T.bg, minHeight: '100svh' }}>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem' }}>
        <button onClick={() => setMode('expense')}
          style={{ flex: 1, padding: '0.6rem', border: `1.5px solid ${mode === 'expense' ? T.lime : T.border}`, borderRadius: 10, background: mode === 'expense' ? `${T.lime}18` : T.card, color: mode === 'expense' ? T.lime : T.slate, fontWeight: mode === 'expense' ? 800 : 400, fontSize: '0.85rem', cursor: 'pointer', fontFamily: 'inherit' }}>
          💸 Gasto
        </button>
        <button onClick={() => setMode('transfer')}
          style={{ flex: 1, padding: '0.6rem', border: `1.5px solid ${mode === 'transfer' ? T.lime : T.border}`, borderRadius: 10, background: mode === 'transfer' ? `${T.lime}18` : T.card, color: mode === 'transfer' ? T.lime : T.slate, fontWeight: mode === 'transfer' ? 800 : 400, fontSize: '0.85rem', cursor: 'pointer', fontFamily: 'inherit' }}>
          🔁 Transferencia
        </button>
      </div>
      ...
      {mode === 'expense' ? (
        ...
        <div style={{ background: T.card, borderRadius: 16, padding: '1.25rem', border: `1px solid ${T.border}` }}>
          <div style={{ fontSize: '0.7rem', color: T.slate, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '0.75rem' }}>
            Mover dinero a una meta — {fmtKey(todayKey())}
          </div>
          {goals.length === 0 ? (
            <div style={{ color: T.slate, fontSize: '0.85rem', textAlign: 'center', padding: '1rem 0' }}>
              Todavía no tienes metas creadas. Ve a la pestaña 🏆 Metas para crear una primero.
            </div>
          ) : (
            <>
              <select value={transferGoalId} onChange={(e) => setTransferGoalId(e.target.value)} style={{ ...inputStyle, marginBottom: '0.75rem', cursor: 'pointer' }}>
                {goals.map((g) => <option key={g.id} value={g.id}>{g.name}</option>)}
              </select>
              <input placeholder="Nota (opcional)" value={transferNote} autoComplete="off"
                onChange={(e) => setTransferNote(e.target.value)} style={{ ...inputStyle, marginBottom: '0.75rem' }} />
              <div style={{ position: 'relative', marginBottom: '1rem' }}>
                <span style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: T.lime, fontWeight: 800 }}>$</span>
              ...
              <button onClick={handleTransferSubmit} style={{ width: '100%', padding: '0.9rem', background: T.lime, border: 'none', borderRadius: 12, color: T.bg, fontWeight: 900, fontSize: '1rem', cursor: 'pointer', fontFamily: 'inherit' }}>
                Transferir
              </button>
              <div style={{ fontSize: '0.65rem', color: T.slate, marginTop: '0.6rem', textAlign: 'center' }}>
                Esto no cuenta como gasto — solo mueve dinero de tu disponible hacia la meta.
              </div>
            </>
          )}
        </div>
...
function GoalView({ goals, setGoals, persistGoals, showToast, T }) {
  ...
  function startEdit(g) {
    setEditingId(g.id)
    setEditForm({ name: g.name, target: String(g.target), current: String(g.current) })
  }
  function saveEdit(id) {
    const target = parseFloat(editForm.target)
    const current = parseFloat(editForm.current)
    if (!editForm.name.trim() || isNaN(target) || target <= 0 || isNaN(current) || current < 0) return
    const updated = goals.map((g) =>
      g.id === id ? { ...g, name: editForm.name.trim(), target, current } : g
    )
    setGoals(updated)
    persistGoals(updated)
    setEditingId(null)
      ...
      {goals.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          {goals.map((g) => {
            ...
            if (isEditing) {
              return (
                <div key={g.id} style={{ background: T.card, borderRadius: 14, padding: '1rem', border: `1.5px solid ${T.lime}` }}>
                  <div style={{ fontSize: '0.7rem', color: T.slate, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '0.6rem' }}>Editando meta</div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginBottom: '0.75rem' }}>
                    <input value={editForm.name} onChange={(e) => setEditForm((f) => ({ ...f, name: e.target.value }))} placeholder="Nombre" style={inputStyle} />
                    <div>
                      <div style={{ fontSize: '0.65rem', color: T.slate, marginBottom: 4 }}>Monto objetivo</div>
                      <input type="number" value={editForm.target} onChange={(e) => setEditForm((f) => ({ ...f, target: e.target.value }))} style={inputStyle} />
                    </div>
                    <div>
                      <div style={{ fontSize: '0.65rem', color: T.slate, marginBottom: 4 }}>Monto actual ahorrado</div>
                      <input type="number" value={editForm.current} onChange={(e) => setEditForm((f) => ({ ...f, current: e.target.value }))} style={inputStyle} />
                    </div>
                  </div>
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    <button onClick={() => setEditingId(null)} style={{ flex: 1, padding: '0.7rem', background: T.border, border: 'none', borderRadius: 10, color: T.slate, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>Cancelar</button>
                    <button onClick={() => saveEdit(g.id)} style={{ flex: 2, padding: '0.7rem', background: T.lime, border: 'none', borderRadius: 10, color: T.bg, fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit' }}>Guardar</button>
                  </div>
                </div>
              )
            }
            ...
            return (
              <div key={g.id} style={{ background: T.card, borderRadius: 14, padding: '1rem', border: `1px solid ${T.border}` }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    <button onClick={() => startEdit(g)} style={{ background: 'none', border: 'none', color: T.lime, cursor: 'pointer', fontSize: '0.9rem' }}>✏️</button>
                ...
                <div style={{ display: 'flex', gap: '0.4rem' }}>
                  ...
                  <button onClick={() => adjustGoal(g.id, 'subtract')} style={{ padding: '0.5rem 0.75rem', background: T.danger, border: 'none', borderRadius: 10, color: 'white', fontWeight: 700, cursor: 'pointer', fontSize: '0.8rem' }}>− Restar</button>
                </div>
              </div>
            )
          })}
...
// ============================================================
// Calcula tendencia por categoría: mes actual vs promedio de períodos previos
// ============================================================
function computeCategoryTrends(expenses, savedPeriods) {
  // Orden cronológico: períodos cerrados más viejos primero, luego el actual
  const chronological = [...savedPeriods].reverse().map((p) => p.expenses || [])
  chronological.push(expenses)
  if (chronological.length < 2) return null // no hay historial suficiente

  const catTotals = {}
  chronological.forEach((periodExpenses, idx) => {
    periodExpenses.forEach((e) => {
      if (!catTotals[e.cat]) catTotals[e.cat] = new Array(chronological.length).fill(0)
      catTotals[e.cat][idx] += e.amount
    })
  })

  const results = []
  Object.entries(catTotals).forEach(([catId, values]) => {
    const current = values[values.length - 1]
    const history = values.slice(0, -1)
    const hasHistory = history.some((v) => v > 0)
    if (current === 0 && !hasHistory) return
    const avg = history.reduce((a, b) => a + b, 0) / history.length
    let pctChange = avg > 0 ? ((current - avg) / avg) * 100 : current > 0 ? 100 : 0
    let direction = 'stable'
    if (pctChange > 8) direction = 'up'
    else if (pctChange < -8) direction = 'down'
    const cat = CATEGORIES.find((c) => c.id === catId) || CATEGORIES[8]
    results.push({ cat, current, avg, pctChange: Math.round(pctChange), direction })
  })
...
function TrendsView({ savedPeriods, expenses, income, T, i, compact }) {
  ...
  const categoryTrends = compact ? null : computeCategoryTrends(expenses, savedPeriods)
  ...
  return (
    <div style={{ padding: compact ? '0.75rem 1rem 1rem' : '1rem 1.25rem', paddingBottom: compact ? undefined : '6rem' }}>
      ...
      {!compact && (
        <>
          <div style={{ fontSize: '0.7rem', color: T.slate, textTransform: 'uppercase', letterSpacing: '0.08em', margin: '1.5rem 0 0.75rem' }}>
            Por categoría — este período vs. tu promedio
          </div>
          {categoryTrends === null ? (
            <div style={{ textAlign: 'center', padding: '1.5rem 0', color: T.slate, fontSize: '0.85rem' }}>
              Cierra al menos un período para ver comparaciones por categoría.
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              {categoryTrends.map(({ cat, current, avg, pctChange, direction }) => {
                const dirColor = direction === 'up' ? T.danger : direction === 'down' ? (T.good || T.lime) : T.slate
                return (
                  <div key={cat.id} style={{ background: T.card, borderRadius: 12, padding: '0.85rem 1rem', border: `1px solid ${T.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <div style={{ color: T.white, fontSize: '0.9rem', fontWeight: 700 }}>{cat.emoji} {cat.label}</div>
                      <div style={{ color: T.slate, fontSize: '0.7rem' }}>{fmt(current)} este período · promedio {fmt(avg)}</div>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: `${dirColor}1a`, borderRadius: 999, padding: '0.3rem 0.6rem' }}>
                      <TrendIcon direction={direction} color={dirColor} size={16} />
                      <span style={{ color: dirColor, fontWeight: 800, fontSize: '0.85rem' }}>{pctChange > 0 ? '+' : ''}{pctChange}%</span>
                    </div>
                  </div>
                )
              })}
...
export default function App() {
  ...
  function persist(overrides = {}) {
    if (!user) return
    const payload = {
      income, period, expenses, transfers, budgetRule, budgetPcts, savedPeriods, goals, themeId, lang,
      ...overrides,
    }
  ...
  const totalSpent = useMemo(
    () => expenses.filter((e) => !(e.cat === 'loan' && e.repaid)).reduce((s, e) => s + e.amount, 0),
    [expenses]
  )
  const totalTransferred = useMemo(() => transfers.reduce((s, t) => s + t.amount, 0), [transfers])
  const remaining = totalIncome - totalSpent - totalTransferred
  ...
  function addTransfer(goalId, amount, note) {
    const amt = parseFloat(amount)
    if (!goalId || isNaN(amt) || amt <= 0) return false
    const goal = goals.find((g) => g.id === goalId)
    if (!goal) return false
    const newTransfer = { id: Date.now(), goalId, amount: amt, note: note || '', date: todayKey() }
    const updatedTransfers = [newTransfer, ...transfers]
    const updatedGoals = goals.map((g) => (g.id === goalId ? { ...g, current: g.current + amt } : g))
    setTransfers(updatedTransfers)
    setGoals(updatedGoals)
    persist({ transfers: updatedTransfers, goals: updatedGoals })
    showToast(`Transferido a "${goal.name}" 💸`)
    return true
  }
  ...
  function toggleLoanRepaid(id) {
    const updated = expenses.map((e) => (e.id === id ? { ...e, repaid: !e.repaid } : e))
    setExpenses(updated)
    persist({ expenses: updated })
    const item = updated.find((e) => e.id === id)
    showToast(item?.repaid ? 'Préstamo marcado como devuelto ✅' : 'Préstamo marcado como pendiente')
  }
  // Solo informativo: marca un préstamo de un período YA CERRADO como devuelto,
  // sin recalcular montos de ese período (esos números quedan fijos en la historia).
  function toggleSavedLoanRepaid(periodId, expenseId) {
    const updatedPeriods = savedPeriods.map((p) => {
      if (p.id !== periodId) return p
      return {
        ...p,
        expenses: p.expenses.map((e) => (e.id === expenseId ? { ...e, repaidInfo: !e.repaidInfo } : e)),
      }
    })
    setSavedPeriods(updatedPeriods)
    persist({ savedPeriods: updatedPeriods })
  }
  ...
  function resetPeriod() {
    if (expenses.length > 0 || transfers.length > 0) {
      ...
      const periodRecord = {
        ...
        expenses: [...expenses], transfers: [...transfers],
      }
      const updatedPeriods = [periodRecord, ...savedPeriods]
      setSavedPeriods(updatedPeriods)
      setExpenses([])
      setTransfers([])
      persist({ expenses: [], transfers: [], savedPeriods: updatedPeriods })
    }
    setShowReset(false)
        ...
        <div style={{ background: T.card, padding: '1.25rem 1.25rem 1rem', borderBottom: `1px solid ${T.border}` }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: '1rem' }}>
            ...
            <div style={{ display: 'flex', gap: 6, justifyContent: 'center', flexWrap: 'wrap', marginTop: 6 }}>
              {totalTransferred > 0 && (
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: T.elevated, borderRadius: 20, padding: '0.25rem 0.75rem' }}>
                  <span style={{ fontSize: '0.7rem', color: T.slate }}>🔁 Transferido:</span>
                  <span style={{ fontSize: '0.8rem', fontWeight: 800, color: T.lime }}>{fmt(totalTransferred)}</span>
                </div>
              )}
            ...
            {expenses.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.45rem' }}>
                {expenses.filter((e) =>
                  ...
                  const isLoan = e.cat === 'loan'
                  const loanColor = isLoan ? (e.repaid ? (T.good || T.lime) : T.danger) : cat.color
                  return (
                    <div key={e.id} style={{ background: T.card, borderRadius: 11, padding: '0.65rem 0.75rem', borderLeft: `3px solid ${loanColor}`, opacity: isLoan && e.repaid ? 0.65 : 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.65rem' }}>
                        {isLoan && (
                          <button onClick={() => toggleLoanRepaid(e.id)}
                            style={{ width: 22, height: 22, borderRadius: 6, border: `1.5px solid ${loanColor}`, background: e.repaid ? loanColor : 'transparent', color: T.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0, fontSize: '0.7rem' }}>
                            {e.repaid ? '✓' : ''}
                          </button>
                        )}
                        <span style={{ fontSize: '1rem' }}>{cat.emoji}</span>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontWeight: 600, fontSize: '0.85rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', textDecoration: isLoan && e.repaid ? 'line-through' : 'none' }}>{e.desc}</div>
                          {e.note && <div style={{ fontSize: '0.65rem', color: T.lime }}>📝 {e.note}</div>}
                          <div style={{ fontSize: '0.65rem', color: T.slate }}>{fmtKey(e.date)} · {cat.label}{isLoan && (e.repaid ? ' · Devuelto' : ' · Pendiente')}</div>
                        </div>
                        <div style={{ fontWeight: 700, color: loanColor, fontSize: '0.85rem', whiteSpace: 'nowrap' }}>-{fmt(e.amount)}</div>
                        ...
                        <div style={{ borderTop: `1px solid ${T.border}`, padding: '0.75rem 1rem' }}>
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                            {[...pExp].sort((a, b) => b.date.localeCompare(a.date)).map((e) => {
                              const cat = CATEGORIES.find((c) => c.id === e.cat) || CATEGORIES[8]
                              const isLoan = e.cat === 'loan'
                              const loanColor = isLoan ? (e.repaidInfo ? (T.good || T.lime) : T.danger) : cat.color
                              return (
                                <div key={e.id} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: T.elevated, borderRadius: 9, padding: '0.55rem 0.65rem', borderLeft: `2px solid ${loanColor}`, opacity: isLoan && e.repaidInfo ? 0.65 : 1 }}>
                                  {isLoan && (
                                    <button onClick={() => toggleSavedLoanRepaid(id, e.id)}
                                      style={{ width: 20, height: 20, borderRadius: 5, border: `1.5px solid ${loanColor}`, background: e.repaidInfo ? loanColor : 'transparent', color: T.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0, fontSize: '0.65rem' }}>
                                      {e.repaidInfo ? '✓' : ''}
                                    </button>
                                  )}
                                  <span style={{ fontSize: '0.9rem' }}>{cat.emoji}</span>
                                  <div style={{ flex: 1, minWidth: 0 }}>
                                    <div style={{ fontWeight: 600, fontSize: '0.8rem', color: T.white, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', textDecoration: isLoan && e.repaidInfo ? 'line-through' : 'none' }}>{e.desc}</div>
                                    <div style={{ fontSize: '0.6rem', color: T.slate }}>{fmtKey(e.date)}{isLoan && (e.repaidInfo ? ' · Devuelto' : ' · No devuelto a tiempo')}</div>
                                  </div>
                                  <span style={{ fontWeight: 700, color: loanColor, fontSize: '0.8rem', whiteSpace: 'nowrap' }}>-{fmt(e.amount)}</span>
                                </div>
                              )
                            })}
                          </div>
  ...
  function BudgetView() {
    ...
    const spentPersonal = expenses
      .filter((e) => PERSONAL_CATS.includes(e.cat) && !(e.cat === 'loan' && e.repaid))
  ...
  return (
    <div style={{ minHeight: '100svh', background: T.bg, fontFamily: "'Inter',sans-serif", color: T.white, maxWidth: 480, margin: '0 auto', position: 'relative' }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 400, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.25rem' }}>
          <div style={{ background: T.card, borderRadius: 20, padding: '1.5rem', width: '100%', maxWidth: 340, border: `1px solid ${T.lime}40`, textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              ...
              <button onClick={() => {
                if (expenses.length > 0 || transfers.length > 0) {
                  const periodRecord = { id: Date.now(), label: prevMonthLabel, date: todayKey(), income: totalIncome, spent: totalSpent, saved: totalIncome - totalSpent, expenses: [...expenses], transfers: [...transfers] }
                  const updatedPeriods = [periodRecord, ...savedPeriods]
                  setSavedPeriods(updatedPeriods)
                  setExpenses([])
                  setTransfers([])
      ...
      <div style={{ paddingBottom: 70 }}>
        ...
        {tab === 'add' && <AddView form={form} setForm={setForm} addExpense={addExpense} addTransfer={addTransfer} goals={goals} etMood={etMood} remColor={remColor} remaining={remaining} i={i} T={T} />}
      </div>
